...
public class SFA {
	...
	public void step() {
		 ...
		 * 
		 * The only element of Bn-1 is cut from Bn-1 and pasted into An-1 on the right
		 * side. Thus set An-1 becomes An . At the start Bstart = <>. Since there is no
		 * element in Bstart to cut A1 is an empty walkset.
		 */
		 ...
		 * 
		 * The leftmost element of Cn-1 is cut and pasted into Bn-1 left empty be the
		 * update of A. Thus Bn-1 again contains one element that is the step number and
		 * thus becomes Bn. At the start Bstart = <> therefore as B is filled for the
		 * first time it becomes B1.
		 */
		 ...
		 * 
		 * This has already taken place during the update of B by cutting the leftmost
		 * element of Cn-1. Thus Cn-1 has become Cn.
		 */
		// update walkset Cn
		 ...
		 * 
		 * The only element of BPn-1 is cut from BPn-1 and pasted into APn-1. The new
		 * element has to become the rightmost element to keep AP in match with A. Thus
		 * APn-1 becomes APn. At the start BPstart = <> therefore AP1 is an empty set,
		 * since there is no element in BPstart to cut.
		 */
		 ...
		 * 
		 * The leftmost element of CPn-1 is copied (not cut ! see lemma) from CPn-1 and
		 * pasted into BPn-1 left empty by the update of AP. Thus BPn-1 again contains
		 * one element that is the type of the step number and thus becomes BPn. At the
		 * start BPstart = <>, therefore as BP n-1 is filled for the first time it
		 * becomes BP1.
		 */
		 ...
		 * information is contained in BPn. If BPn contains the type element M then the
		 * current step number is not prime. If BPn contains the type element L then the
		 * current step number is prime. This is denoted by changing L into P in BPn
		 * (with exception of number one which turns L into symbol 1)
		 */
		if (walksetBn.getN() == 1) {
		 ...
		 * 
		 * Fractal procedure: move As can be seen in the description "Update of pattern
		 * walkset BPn-1 to BPn" the type information of the step number n is not cut
		 * from CPn-1. Instead this leftmost type information is moved to the rightmost
		 * place of the periodic term. Thus CPn-1 becomes CPn (see lemma).
		 * 
		 * Update pattern walkset CPn-1 to CPn when n is prime
		 * 
		 * Fractal procedure: move The first procedure is the same as for "n is not
		 * prime"
		 */
		...
		if (walksetBPn.getN().equals(Symbol.P)) {
			 ...
			 * 
			 * The sieve size is increased by copying the pattern and pasting it n-1 times
			 ...
			 * 
			 * The types of all numbers x for which applies x·n with (x isElementOf N AND x
			 * > 1 AND x·n lessThan sievesizeAtN + n) in CPn-1 are turned from undetermined
			 * L types into M types, unless they are already of type M. Thus CPn-1 becomes
			 * CPn
			 */
	...
	public LinkedList<Symbol> getPattern() {
		return this.walksetCPn.getPattern();
	}
